...
import { STATUS_COLORS } from '@/lib/plan-theme';

/** The five statuses, in the plan's own colours (`Legend.tsx` in the authoring tool, minus its filter). */
const ROWS: { key: 'live' | 'ending' | 'soon' | 'free' | 'service'; bg: string }[] = [
    ...
    >
      {ROWS.map((row) => (
        <div key={row.key} className="legend-row" style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '3px 4px' }}>
          <span className="swatch" style={{ width: 14, height: 14, borderRadius: 3, background: row.bg, flex: 'none' }} />
