...
## Repo map
`apps/web` — Next.js 15 App Router UI (board, 2.5D map, overlays). `services/api` — Go service (`cmd/api`, `cmd/seed`, `internal/{domain,engine,schedule,repo,httpapi,realtime,scheduler,clock,config,seed}`, goose `migrations/`). `packages/contracts` — `openapi.yaml` (single source of truth for the HTTP contract) plus the committed `src/types.gen.ts` produced by `openapi-typescript`; Go types are generated from the same file into `services/api/internal/httpapi/gen.go` by `oapi-codegen`. `packages/map-data` — `svg/floor-{1,2}.svg` (geometry source of truth, traced from the real plans) → `scripts/svg2map.ts` → committed `building-a.json`, validated against `schema.json`; consumed by both the web app and `cmd/seed`. `infra` — docker-compose, Caddyfile, `.env.example`, demo scripts. `docs` — ARCHITECTURE.md, STATUS.md, design exports and prompts.
